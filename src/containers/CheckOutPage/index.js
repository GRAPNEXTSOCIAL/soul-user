...
const Address = ({
  ...
  const deleteSelectedAddress = (id) =>{
    console.log(id);
          ...
          <div style={{ width: "100%" }}>
            <div className="addressDetails">
              ...
              {adr.selected && (
                ...
                <Anchor
                  name="DELETE"
                  onClick={()=>deleteSelectedAddress(adr._id)}
                  style={{
                    fontWeight: "500",
                    color: "#cb8364",
                    padding: "10px"
...
const CheckOutPage = (props) => {
  ...
  const confirmCOD = () => {
    confirmAlert({
      message: 'Do you really want to proceed with COD or pay online for a hassle free experience?',
      buttons: [
        {
          label: 'Proceed with COD',
          onClick: () => onConfirmOrder(true)
        },
        {
          label: "No, I'll pay online!",
          onClick: () => onConfirmOrder(false)
        }
      ]
    });
  }
  const onConfirmOrder = (isConfirm) => {
    if(!isConfirm) return;
